...
initFrame:SetScript("OnEvent", function(self)
    ...
    -- The spell list can run to hundreds of rows, so the search box filters
    -- this cache instead of re-enumerating on every keystroke.
    local pickerLists = {}

    -- Wrath exposes the backpack as bag 0 and has no reagent bag or
    -- Enum.BagIndex. Keep the newer constants optional so this same walk also
    -- remains correct on clients which do provide them.
    local FIRST_CARRIED_BAG =
        (Enum and Enum.BagIndex and Enum.BagIndex.Backpack) or 0
    local REAGENT_BAG = Enum and Enum.BagIndex and Enum.BagIndex.ReagentBag
    local function ScanCarriedBags(scan)
        for bag = FIRST_CARRIED_BAG, (NUM_BAG_SLOTS or 4) do scan(bag) end
        if REAGENT_BAG
           and (REAGENT_BAG < FIRST_CARRIED_BAG or REAGENT_BAG > (NUM_BAG_SLOTS or 4)) then
            scan(REAGENT_BAG)
        end
    end
    ...
    --  Enumeration
    --
    --  Strictly read-only, every list. Filtering happens in Lua here so opening
    --  the picker never mutates any of the game's own UI state.
    ---------------------------------------------------------------------------
    ...
    local function ItemEntries()
        ...
        local function ScanBag(bag)
            for slot = 1, C_Container.GetContainerNumSlots(bag) do
                ...
                local itemName = itemID and
                    (info.itemName or C_Item.GetItemInfo(itemID))
                if itemID and not seen[itemID] and itemName
                   and C_Item.GetItemSpell(itemID) then
                    seen[itemID] = true
                    out[#out + 1] = { icon = info.iconFileID, name = itemName,
                        slot = { kind = "item", id = itemID, name = itemName } }
                end
            end
        end
        ScanCarriedBags(ScanBag)
        return out
    end
    ...
    local function EnsurePickerMenu()
        ...
        bAr:SetTexture("Interface\\AddOns\\EllesmereUI\\media\\icons\\eui-arrow-left.tga")
    ...
    -- The base item plus its two expansion siblings.
    ...
    -- in the quest item CLASS -- a trinket or another special-use item handed
    -- out for one quest reads as its own class and would be missed by the bag
    -- walk alone.
    ...
    local function RebuildMenuRows()
        ...
        for i = 1, count do
            ...
            if not itm then
                ...
                xIcon:SetTexture("Interface\\AddOns\\EllesmereUI\\media\\icons\\eui-close.tga")
                xBtn:SetAlpha(0.4)
                itm._xBtn = xBtn
                ...
                editIcon:SetTexture("Interface\\AddOns\\EllesmereUI\\media\\icons\\eui-edit.tga")
    ...
    -- Retail supplies these helpers to Blizzard's keybinding widgets, but the
    -- 3.3.5 client does not. Keep the native path when it exists and provide
    -- the small subset Quickdraw's capture button needs on legacy clients.
    ...
    local function ConvertBindingInput(input)
        if GetConvertedKeyOrButton then
            return GetConvertedKeyOrButton(input)
        end
        local converted = MOUSE_BUTTON_KEYS[input]
        if converted then return converted end
        local number = type(input) == "string" and input:match("^Button(%d+)$")
        return number and ("BUTTON" .. number) or input
    end

    local function BindingInputIsIgnored(input)
        if IsKeyPressIgnoredForBinding then
            return IsKeyPressIgnoredForBinding(input)
        end
        return input == "LSHIFT" or input == "RSHIFT"
            or input == "LCTRL" or input == "RCTRL"
            or input == "LALT" or input == "RALT"
            or input == "LMETA" or input == "RMETA"
    end

    local function BuildBindingChord(input)
        if CreateKeyChordStringUsingMetaKeyState then
            return CreateKeyChordStringUsingMetaKeyState(input)
        end
        local modifiers = ""
    ...
    local function BuildKeybindButton(rgn, spec)
        ...
        kbBtn:SetScript("OnClick", function(self, button)
            ...
            if listening and ((button ~= "LeftButton" and button ~= "RightButton")
                ...
                Commit(BuildBindingChord(ConvertBindingInput(button)))
                RefreshLabel()
                return
            end
        ...
        row, h = W:DualRow(parent, y,
            ...
            { type="toggle", text="Dim Unusable Entries", noCapture=true,
              ...
              tooltip="Tint an entry that would do nothing right now: red when the "
                      ...
                      .."reason. Spells and items only -- a macro's usability "
