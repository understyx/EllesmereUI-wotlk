...
PallyPower = AceLibrary("AceAddon-2.0"):new("AceConsole-2.0","AceEvent-2.0","AceDebug-2.0")
EllesmereUI.PallyPower = PallyPower
...
PallyPower.assignments = {}
PallyPower.normalAssignments = {}
PallyPower.auraAssignments = {}
...
local function EnsureFlavorBuckets(store)
	store["Vanilla"] = store["Vanilla"] or {}
	store["TBC"] = store["TBC"] or {}
	store["Wrath"] = store["Wrath"] or {}
end

function PallyPower:ActivateProfile()
	if not moduleDB or not moduleDB.profile then return end
	local profile = moduleDB.profile
	profile.settings = profile.settings or {}
	profile.assignments = profile.assignments or {}
	profile.normalAssignments = profile.normalAssignments or {}
	profile.auraAssignments = profile.auraAssignments or {}
	profile.savedPresets = profile.savedPresets or {}
	EnsureFlavorBuckets(profile.assignments)
	EnsureFlavorBuckets(profile.normalAssignments)
	EnsureFlavorBuckets(profile.auraAssignments)

	self.opt = profile.settings
	self.assignments = profile.assignments
	self.normalAssignments = profile.normalAssignments
	self.auraAssignments = profile.auraAssignments
	self.savedPresets = profile.savedPresets
end

-- Public module API for sibling EllesmereUI addons.
function PallyPower:IsReady()
	return moduleDB ~= nil and self.opt ~= nil
end

function PallyPower:IsPlayerPaladin()
	if PP_IsPally == true then return true end
	local _, class = UnitClass("player")
	return class == "PALADIN"
end

function PallyPower:GetProfile()
	return moduleDB and moduleDB.profile
end

function PallyPower:GetOptions()
	return self.opt
end

function PallyPower:GetFlavor()
	return flavor
end

function PallyPower:GetAssignments()
	return self.assignments[flavor], self.normalAssignments[flavor], self.auraAssignments[flavor]
end

function PallyPower:GetAssignedBlessing(paladinName, classID, playerName)
	if type(classID) == "string" then
		classID = self.ClassToID and self.ClassToID[classID]
	end
	if not paladinName or not classID then return nil end
	local classAssignments = self.assignments[flavor] or {}
	local normalAssignments = self.normalAssignments[flavor] or {}
	local blessingID
	if playerName and normalAssignments[paladinName]
	   and normalAssignments[paladinName][classID] then
		blessingID = normalAssignments[paladinName][classID][playerName]
	end
	if blessingID == nil and classAssignments[paladinName] then
		blessingID = classAssignments[paladinName][classID]
	end
	if blessingID == nil then return nil end
	return blessingID, self.Spells and self.Spells[blessingID],
		self.GSpells and self.GSpells[blessingID]
end

function PallyPower:GetAssignedBlessingForUnit(paladinName, unit)
	if not unit or not UnitExists(unit) then return nil end
	local playerName = UnitName(unit)
	local _, classToken = UnitClass(unit)
	return self:GetAssignedBlessing(paladinName, classToken, playerName)
end

function PallyPower:HasAssignmentsForPaladin(paladinName)
	if not paladinName then return false end
	local classAssignments = (self.assignments[flavor] or {})[paladinName] or {}
	for _, blessingID in pairs(classAssignments) do
		if type(blessingID) == "number" and blessingID > 0 then return true end
	end
	local normalAssignments = (self.normalAssignments[flavor] or {})[paladinName] or {}
	for _, players in pairs(normalAssignments) do
		for _, blessingID in pairs(players) do
			if type(blessingID) == "number" and blessingID > 0 then return true end
		end
	end
	return false
...
function PallyPower:HasBlessingAssignment(paladinName, wantedBlessingID)
	if not paladinName or not wantedBlessingID then return false end
	local classAssignments = (self.assignments[flavor] or {})[paladinName] or {}
	for _, blessingID in pairs(classAssignments) do
		if blessingID == wantedBlessingID then return true end
	end
	local normalAssignments = (self.normalAssignments[flavor] or {})[paladinName] or {}
	for _, players in pairs(normalAssignments) do
		for _, blessingID in pairs(players) do
...
function PallyPower:SetClassAssignment(paladinName, classID, blessingID)
	...
	if not paladinName or not classID or classID < 1 or classID > PALLYPOWER_MAXCLASSES then
		return false
	end
	if blessingID < 0 or blessingID > PALLYPOWER_MAXBLESSINGS then return false end
	if not self:CanControl(paladinName) then return false end
	local paladin = AllPallys[paladinName]
	if blessingID > 0 and (not paladin or not paladin[blessingID]
	   or (paladin[blessingID].rank or 0) == 0) then
		return false
	end

	self.assignments[flavor][paladinName] = self.assignments[flavor][paladinName] or {}
	self.assignments[flavor][paladinName][classID] = blessingID
	self:SendMessage("ASSIGN " .. paladinName .. " " .. classID .. " " .. blessingID)
	self:UpdateLayout()
...
function PallyPower:GetAuraAssignment(paladinName)
	local auraAssignments = self.auraAssignments[flavor] or {}
	local auraID = auraAssignments[paladinName]
	if auraID == nil then return nil end
	return auraID, self.Auras and self.Auras[auraID]
end

function PallyPower:GetRoster()
	return AllPallys
end

function PallyPower:RequestRefresh()
...
function PallyPower:ResetProfile()
	if not moduleDB then return end
	moduleDB:ResetProfile()
...
function PallyPower:RefreshProfile()
	if not moduleDB then return end
	self:ActivateProfile()
	if InCombatLockdown and InCombatLockdown() then
		self._profileRefreshPending = true
...
function PallyPower:OnInitialize()
	moduleDB = EllesmereUI.Lite.NewDB("EllesmereUIPallyPowerDB", {
		profile = {
			settings = PALLYPOWER_DEFAULT_VALUES,
			assignments = { Vanilla = {}, TBC = {}, Wrath = {} },
			normalAssignments = { Vanilla = {}, TBC = {}, Wrath = {} },
			auraAssignments = { Vanilla = {}, TBC = {}, Wrath = {} },
			savedPresets = {},
		},
	})
	self.db = moduleDB
...
function PallyPower:OnEnable()
	-- EllesmereUI may have selected a spec-assigned profile between this
	-- module's ADDON_LOADED initialization and PLAYER_LOGIN.
	self:ActivateProfile()
	-- events
...
function PallyPower:Purge()
	PallyPower.assignments[flavor] = nil
	PallyPower.normalAssignments[flavor] = nil
	PallyPower.auraAssignments[flavor] = nil
	PallyPower.assignments[flavor] = {}
	PallyPower.normalAssignments[flavor] = {}
	PallyPower.auraAssignments[flavor] = {}
end
...
function GetNormalBlessings(pname, class, tname)
	if PallyPower.normalAssignments[flavor][pname] and PallyPower.normalAssignments[flavor][pname][class] then
		local blessing = PallyPower.normalAssignments[flavor][pname][class][tname]
		if blessing then
...
function SetNormalBlessings(pname, class, tname, value)
	if not PallyPower.normalAssignments[flavor][pname] then
		PallyPower.normalAssignments[flavor][pname] = {}
	end
	if not PallyPower.normalAssignments[flavor][pname][class] then
		PallyPower.normalAssignments[flavor][pname][class] = {}
	end
	PallyPower:SendMessage("NASSIGN "..pname.." "..class.." "..tname.." "..value)  
	if value == 0 then value = nil end
	PallyPower.normalAssignments[flavor][pname][class][tname] = value
end
...
function PallyPowerGrid_NormalBlessingMenu(btn, mouseBtn, pname, class)
	...
	elseif (mouseBtn == "RightButton") then
		for pally in pairs(AllPallys) do
			if PallyPower.normalAssignments[flavor][pally] and PallyPower.normalAssignments[flavor][pally][class] and PallyPower.normalAssignments[flavor][pally][class][pname] then
				PallyPower.normalAssignments[flavor][pally][class][pname] = nil
...
function PallyPowerGridButton_OnClick(btn, mouseBtn)
	...
	if (mouseBtn == "RightButton") then
		PallyPower.assignments[flavor][pname][class] = 0
...
function PallyPowerConfigGrid_Update()
	...
	if PallyPowerConfigFrame:IsVisible() then
		...
		for i, name in pairs(SyncList) do
			...
			local BuffInfo = PallyPower.assignments[flavor][name]
			local NormalBuffInfo = PallyPower.normalAssignments[flavor][name]
			...
			local aura = PallyPower.auraAssignments[flavor][name]
	...
	if not type then
		...
		if PallyPower:CheckRaidLeader(self.player) then
			...
			for name in pairs(AllPallys) do
				...
				for id = 1, PALLYPOWER_MAXCLASSES do
					local bid = PallyPower.assignments[flavor][name][id]
...
function PallyPower:PerformCycle(name, class, skipzero)
	...
	if not PallyPower.assignments[flavor][name] then
		PallyPower.assignments[flavor][name] = {}
	end
	if not PallyPower.assignments[flavor][name][class] then
		cur=0
	else
		cur=PallyPower.assignments[flavor][name][class]
	end
	PallyPower.assignments[flavor][name][class] = 0
	...
	if shift then
		for test = 1, PALLYPOWER_MAXCLASSES do
			PallyPower.assignments[flavor][name][test] = cur
		end
		PallyPower:SendMessage("MASSIGN "..name.." "..cur)
	else
		PallyPower.assignments[flavor][name][class] = cur
...
function PallyPower:PerformPlayerCycle(arg1, pname, class)
	...
	if PallyPower.normalAssignments[flavor][playername] and PallyPower.normalAssignments[flavor][playername][class] and PallyPower.normalAssignments[flavor][playername][class][pname] then
		blessing = PallyPower.normalAssignments[flavor][playername][class][pname]
	end
...
function PallyPower:AssignPlayerAsClass(pname, pclass, tclass)
	...
	for pally, classes in pairs(PallyPower.assignments[flavor]) do
	...
	for pally, info in pairs(AllPallys) do
		if PallyPower.assignments[flavor][pally] and PallyPower.assignments[flavor][pally][pclass] then
			local blessing = PallyPower.assignments[flavor][pally][pclass]
	...
	for index, blessing in pairs(targetsorted) do
		if greater[blessing] then
			...
			if PallyPower.normalAssignments[flavor][pally] and 
			   PallyPower.normalAssignments[flavor][pally][pclass] and 
			   PallyPower.normalAssignments[flavor][pally][pclass][pname] then
...
function PallyPower:NeedsBuff(class, test, playerName)
	...
	if playerName then
		for pname, classes in pairs(PallyPower.normalAssignments[flavor]) do
	...
	for name, skills in pairs(PallyPower.assignments[flavor]) do
...
--    local name = UnitName("player")
	...
	if not PallyPower.assignments[flavor][self.player] then
		PallyPower.assignments[flavor][self.player] = {}
		for i = 1, PALLYPOWER_MAXCLASSES do
			PallyPower.assignments[flavor][self.player][i] = 0
		end
	end

	local BuffInfo = PallyPower.assignments[flavor][self.player]
	...
	if not PallyPower.auraAssignments[flavor][self.player] then
		PallyPower.auraAssignments[flavor][self.player] = 0
	end
	
	s = s .. "@" .. PallyPower.auraAssignments[flavor][self.player]
	
	self:SendMessage("ASELF " .. s)

	local AssignList = {}
	local inraid = GetNumRaidMembers() > 0
	if PallyPower.normalAssignments[flavor][self.player] then
		for class_id, tnames in pairs(PallyPower.normalAssignments[flavor][self.player]) do
			for tname, blessing_id in pairs(tnames) do
...
function PallyPower:ACTIVE_TALENT_GROUP_CHANGED()
	...
	if (class == "PALADIN") then
		...
		self.opt.sets[old].aura = PallyPower.auraAssignments[flavor][self.player]
		PallyPower.auraAssignments[flavor][self.player] = self.opt.sets[new].aura
			
		self.opt.sets[old].rf = self.opt.rf
		self.opt.rf = self.opt.sets[new].rf
			
		for i = 1, PALLYPOWER_MAXCLASSES do
			self.opt.sets[old].buffs[i] = PallyPower.assignments[flavor][self.player][i]
			PallyPower.assignments[flavor][self.player][i] = self.opt.sets[new].buffs[i]
		end
		PallyPower:UpdateLayout()
	end
end
...
function PallyPower:PLAYER_REGEN_ENABLED()
	if self._profileRefreshPending then
		self._profileRefreshPending = nil
		self:UpdateLayout()
		self:UpdateRoster()
		self:BindKeys()
	elseif PP_IsPally then
		self:UpdateLayout()
	end
end
...
function PallyPower:ClearAssignments(sender)
	...
	for name, skills in pairs(PallyPower.assignments[flavor]) do
		if leader or name == sender then
			...
			for i = 1, PALLYPOWER_MAXCLASSES do
				PallyPower.assignments[flavor][name][i] = 0
			end
		end
	end
	for pname, classes in pairs(PallyPower.normalAssignments[flavor]) do
	...
	for name, auras in pairs(PallyPower.auraAssignments[flavor]) do
		if leader or name == sender then
			PallyPower.auraAssignments[flavor][name] = 0
		end
	end
end
...
function PallyPower:ParseMessage(sender, msg)
	...
	if sfind(msg, "^SELF") then
		PallyPower.normalAssignments[flavor][sender] = {}
		PallyPower.assignments[flavor][sender] = {}
		AllPallys[sender] = {}
		...
		if assign then
			for i = 1, PALLYPOWER_MAXCLASSES do
				...
				PallyPower.assignments[flavor][sender][i] = tmp + 0
			end
		end
	end
	...
	if sfind(msg, "^ASSIGN") then
		...
		class = class + 0
		skill = skill + 0
		PallyPower.assignments[flavor][name][class] = skill
	end
	...
	if sfind(msg, "^NASSIGN") then
		for pname, class, tname, skill in string.gmatch(ssub(msg, 9), "([^@]*) ([^@]*) ([^@]*) ([^@]*)") do
			...
			PallyPower.normalAssignments[flavor][pname][class][tname] = skill
		end
	end
	...
	if sfind(msg, "^MASSIGN") then
		...
		skill = skill + 0
		for i = 1, PALLYPOWER_MAXCLASSES do
			PallyPower.assignments[flavor][name][i] = skill
		end
	end
	...
	if sfind(msg, "^ASELF") then
		PallyPower.auraAssignments[flavor][sender] = 0
		...
		if assign then
			...
			PallyPower.auraAssignments[flavor][sender] = assign + 0
		end
	end
	...
	if self.RefreshAssignmentOptions then
		self:RefreshAssignmentOptions(sfind(msg, "^SELF") ~= nil)
	end
...
--						end
	...
	self:UpdateLayout()
	if self.RefreshAssignmentOptions then self:RefreshAssignmentOptions(true) end
...
function PallyPower:UpdateLayout()
	...
	if self.opt.display.flyoutEdge then
		...
		elseif (classAlignment == "Top Left") then
			x = x * -1
			point = "BOTTOMRIGHT"
			pointOpposite = "TOPRIGHT"
		elseif (classAlignment == "Bottom Left") then
			x = x * -1
			y = y * -1
			point = "TOPRIGHT"
			pointOpposite = "BOTTOMRIGHT"
		elseif (classAlignment == "Bottom Right") then
			y = y * -1
			point = "TOPLEFT"
		...
		for cbNum = 1, PALLYPOWER_MAXCLASSES do -- position class buttons
			...
			for pbNum = 1, PALLYPOWER_MAXPERCLASS do -- position player buttons
				...
				if (playerAlignment == "bottom") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x,
										cButton.y - pbNum * (buttonHeight + gapping)
									)
				elseif (playerAlignment == "left") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x - pbNum * (buttonWidth + gapping),
										cButton.y
									)
				elseif (playerAlignment == "right") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x + pbNum * (buttonWidth + gapping),
										cButton.y
									)
				elseif (playerAlignment == "top") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x,
										cButton.y + pbNum * (buttonHeight + gapping)
									)
				elseif (playerAlignment == "compact-right") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x + (buttonWidth + gapping),
										cButton.y + (pbNum - 1) * (buttonHeight + gapping)
									)
				elseif (playerAlignment == "compact-left") then
					pButton:SetPoint(	point, self.Header, "CENTER",
										cButton.x - (buttonWidth + gapping),
										cButton.y + (pbNum - 1) * (buttonHeight + gapping)
									)
		...
		PallyPower:UpdateAuraButton(PallyPower.auraAssignments[flavor][self.player])
	...
	if classlist[classID] then
		for pbNum = 1, math.min(classlist[classID], PALLYPOWER_MAXPERCLASS) do
			...
			if unit and unit.name and
				PallyPower.normalAssignments[flavor][self.player] and
				PallyPower.normalAssignments[flavor][self.player][classID] and
				PallyPower.normalAssignments[flavor][self.player][classID][unit.name] then
					nbcount = nbcount+1
			end
		end -- by pbnum
	end
...
function PallyPower:GetSpellID(classID, playerName)
	...
	if playerName and
	   PallyPower.normalAssignments[flavor][self.player] and 
	   PallyPower.normalAssignments[flavor][self.player][classID] and
	   PallyPower.normalAssignments[flavor][self.player][classID][playerName] then
		normal = PallyPower.normalAssignments[flavor][self.player][classID][playerName]
	end
	if PallyPower.assignments[flavor][self.player] and PallyPower.assignments[flavor][self.player][classID] then
		greater = PallyPower.assignments[flavor][self.player][classID]
	end
	if normal == 0 then
		normal = greater
	end
...
function PallyPower:SavePreset(preset)
	...
	PallyPower.savedPresets[preset] = {}
	self:Print("Saving preset: "..preset)
	for name in pairs(AllPallys) do
		self:Print("  Paladin: " .. name)
		PallyPower.savedPresets[preset][name] = {}
	    local i
	    for i = 1, PALLYPOWER_MAXCLASSES do
			if not PallyPower.assignments[flavor][name][i] then
				PallyPower.savedPresets[preset][name][i] = 0
			else
				PallyPower.savedPresets[preset][name][i] = PallyPower.assignments[flavor][name][i]
			end
	    end
	end
	self:Print("Done.")
...
function PallyPower:LoadPreset(preset)
	...
	if PallyPower.savedPresets[preset] then
	    self:Print("Loading preset: "..preset)
		for name in pairs(PallyPower.savedPresets[preset]) do
			if not PallyPower.assignments[flavor][name] then PallyPower.assignments[flavor][name] = {} end
			self:Print("       Paladin: " .. name)
			local i
			for i = 1, PALLYPOWER_MAXCLASSES do
				PallyPower.assignments[flavor][name][i] = PallyPower.savedPresets[preset][name][i]
				PallyPower:SendMessage("ASSIGN "..name.." "..i.." "..PallyPower.savedPresets[preset][name][i]) 
			end 
		end
		self:Print("Done.")
	else
...
function PallyPower:ApplyBackdrop(button, preset)
	button:SetBackdropColor(preset["r"], preset["g"], preset["b"], preset["t"] or preset["a"] or 1)
end
...
--				testtalent = t[i].spelltalents
	...
	--end
	if Buffer ~= "" then
		PallyPower.assignments[flavor][Buffer][class] = buff
...
function PallyPowerAuraButton_OnClick(btn, mouseBtn)
	...
	if (mouseBtn == "RightButton") then
		PallyPower.auraAssignments[flavor][pname] = 0
...
function PallyPower:PerformAuraCycle(name, skipzero)
	if not PallyPower.auraAssignments[flavor][name] then
		PallyPower.auraAssignments[flavor][name] = 0
	end

	local cur = PallyPower.auraAssignments[flavor][name]
	...
	if ( cur == PallyPower.auraAssignments[flavor][name] ) then
	...
	PallyPower.auraAssignments[flavor][name] = cur
...
function PallyPower:PerformAuraCycleBackwards(name, skipzero)
	...
	local cur = PallyPower.auraAssignments[flavor][name] - 1
	...
	for test = cur, 0, -1 do
		if PallyPower:HasAura(name, test) or (test == 0 and not skipzero) then
			PallyPower.auraAssignments[flavor][name] = test
...
function PallyPower:UpdateAuraButton(aura)
	...
	if ( aura and aura > 0 ) then
		for name in pairs(AllPallys) do
			if (name ~= self.player) and (aura == PallyPower.auraAssignments[flavor][name]) then
				tinsert(pallys, name)
			end
		end
		...
		if assignee ~= "" then
			for i, name in pairs(pallys) do
				if assignee == name then
					tremove(pallys, i)
					PallyPower.auraAssignments[flavor][assignee] = aura
