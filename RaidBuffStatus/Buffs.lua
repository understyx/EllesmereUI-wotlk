...
--		chat = BS[974],  -- Earth Shield
	...
	focusmagic = {
		...
		click = function(self, button, down)
			RaidBuffStatus:ButtonClick(self, button, down, "focusmagic", BS[54646], BS[54646], nil, true) -- Focus Magic
		end,
		tip = function(self)
...
--					RaidBuffStatus:Debug("having to call notifyinspect for:" .. unit.unitid)
      ...
      elseif class == "WARLOCK" then
        local spellstone = { 41196, 41195, 41194, 41193, 41192, 41191 }
        local firestone = { 41174, 41173, 40773, 41172, 41171, 41169, 41170 }        
         local stonetype, stonecreate
        if GT:GUIDHasTalent(guid, BS[30108]) then -- Unstable Affliction
          stonetype = spellstone
        else
          stonetype = firestone
        end          
        for _,sid in ipairs(stonetype) do        
          if (IsUsableItem(sid)) then
             bagitem = ITN[sid]
             itemslot = GetInventorySlotInfo("MainHandSlot")
             break
          end
        ...
        if not bagitem then
          itemslot = nil
          if stonetype == spellstone then
            buffspell = BS[47888] -- Create Spellstone
          else
            buffspell = BS[60220] -- Create Firestone
          end
        end
      elseif class == "ROGUE" then
...
--								RaidBuffStatus:Debug(name .. " has " .. v .. " from " .. caster)
	...
	amplifymagic = {
		...
		raidbuff = false,
		whispertobuff = function(reportl, prefix)
			for _, name in pairs(RaidBuffStatus.BF.amplifymagic:buffers()) do
				if RaidBuffStatus.db.profile.WhisperMany and #reportl >= RaidBuffStatus.db.profile.HowMany then
	...
	checkpet = {
		...
		main = function(self, name, class, unit, raid, report)
			...
			elseif class == "MAGE" then
			  -- dirty hack: need to also check for Glyph of Eternal Water
		...
		click = function(self, button, down)
		  ...
		  local summonspell = nil
			...
			elseif class == "WARLOCK" then
			  ...
			  elseif GT:GUIDHasTalent(guid, BS[54037]) then -- Improved Felhunter
	        summonspell = BS[691] -- Summon Felhunter
			  elseif GT:GUIDHasTalent(guid, BS[47220]) or GT:GUIDHasTalent(guid, BS[18694]) then -- Empowered Imp, Improved Imp
