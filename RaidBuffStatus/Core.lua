...
RBS_svnrev = {}
RBS_svnrev["Core.lua"] = select(3,string.find("$Revision: 389 $", ".* (.*) .*"))
...
function RaidBuffStatus:ButtonClick(self, button, down, buffcheck, cheapspell, reagentspell, reagent, nonselfbuff, bagitem, itemslot)
	local BF = RaidBuffStatus.BF
...
function RaidBuffStatus:UnitIsMounted(unitid)
  ...
  elseif UnitIsUnit(unitid, "player") then 
      ...
      elseif (expirationTime == 0 and duration == 0 and 
          unitCaster and UnitIsUnit(unitid, unitCaster) and
